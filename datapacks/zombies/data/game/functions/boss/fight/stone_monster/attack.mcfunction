...
execute if score $phase boss matches 2 if score $attack_id boss matches 4 unless entity @e[type=silverfish,tag=boss_block_base] run scoreboard players remove $attack_id boss 1

##If we're in phase 2, and we're doing attack 4 but there are no blocks, summon blocks.
execute if score $phase boss matches 2 if score $attack_id boss matches 3 if entity @e[type=silverfish,tag=boss_block_base] run scoreboard players add $attack_id boss 1


##If we're in phase 2, and we're doing attack 4 but there are no blocks, summon blocks.
execute if score $phase boss matches 2 if score $attack_id boss matches 2 if entity @e[type=silverfish,tag=boss_block_base] run scoreboard players set $attack_id boss 4
