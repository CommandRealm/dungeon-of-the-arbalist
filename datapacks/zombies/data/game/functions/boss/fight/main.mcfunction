...
execute if score $mode settings matches 1 as @a[tag=playing,team=game,gamemode=adventure,tag=in_boss_room,advancements={journal:block=false}] at @s if entity @e[type=zombie,distance=..7] unless score $difficulty settings matches -1 run advancement grant @s only journal:block
