...
##vote end
execute if entity @a[tag=playing,gamemode=adventure,team=game,scores={vote_end=1}] unless entity @a[tag=playing,gamemode=adventure,team=game,scores={vote_end=..0}] if score $time game matches ..994 run scoreboard players set $time game 994
execute as @a[tag=playing,gamemode=adventure,team=game,scores={vote_end=1},tag=!sent_vote_end_message] at @s run function game:default/send_vote_end_message