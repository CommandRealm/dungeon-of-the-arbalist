...
# killing other parts
execute as @e[type=block_display,tag=mob_spawner_part] at @s run scoreboard players operation @s calculate = @s id
execute as @e[type=block_display,tag=mob_spawner_part] at @s run scoreboard players operation @s calculate -= @e[type=magma_cube,tag=temp] id
kill @e[type=block_display,tag=mob_spawner_part,scores={calculate=0}]
