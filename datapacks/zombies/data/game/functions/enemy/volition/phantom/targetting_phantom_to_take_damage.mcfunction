...
execute as @e[type=phantom,tag=phantom_part,scores={calculate=0}] at @s run data merge entity @s {active_effects:[{id:"minecraft:slowness",duration:10}]}
execute as @e[type=phantom,tag=phantom_part,scores={calculate=0}] at @s run data modify entity @s active_effects set from entity @e[type=zombie,tag=temp,tag=phantom_part,limit=1] active_effects
