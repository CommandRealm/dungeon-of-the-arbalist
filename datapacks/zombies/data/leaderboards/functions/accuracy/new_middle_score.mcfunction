...
execute unless entity @e[type=armor_stand,tag=accuracy_leaderboard,tag=temporary_tag_2] run tag @e[type=armor_stand,tag=accuracy_leaderboard,scores={leaderboard_pos=2}] add temporary_tag_2
...
data modify block 0 10 0 front_text.messages[1] set value '[{"text":"","italic":false},{"selector":"@p[tag=temporary_tag]"},{"text":"\'s ","color":"yellow"},{"text":"accuracy ","color":"lime"},{"text":": ","color":"gray"},{"score":{"objective":"accuracy_score","name":"@p[tag=temporary_tag]"},"color":"gold"}]'
