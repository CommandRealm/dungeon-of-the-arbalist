...
##Msg
team join yellow
tellraw @a[x=0,y=66,z=0,distance=..500] ["",{"text":"| ","color":"gray","bold":true},{"selector":"@s"},{"text":" has finished the parkour in ","color":"gold"},{"nbt":"front_text.messages[0]","block":"0 10 0","interpret": true}]
tellraw @s [{"text":"☀","color":"gray"},{"text":" You finished the parkour!","color":"gold"}]

execute if score @s parkour_time matches ..700 unless score $difficulty settings matches -1 unless score $modifiers settings matches 1 run advancement grant @s only advancements:hidden/hidden-parkour
...
##advancement
execute unless score $difficulty settings matches -1 unless score $modifiers settings matches 1 run advancement grant @s only advancements:lobby-parkour
